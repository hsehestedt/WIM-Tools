...
Dim a As Integer ' Throw away variable.
...
Select Case MenuSelection
    ...
    Case 3
        ...
        ' If the user has elected to play a script, then open the script file now and play it back
        ' Note that the "Scripting" procedure has already copied the script to the %temp% location
        ' and named it "WIM_SCRIPT.TXT".
...
Do
    ...
    If ScriptingChoice$ = "R" Then
        Print #5, ":: Path to one or more Windows images:"
...
If Not (_DirExists(SourceFolder$)) Then
    ...
    Color 15
    If ScriptingChoice$ = "R" Then
        Print #5, ":: The above location does not exist or is not valid."
        Print #5, "::"
    End If
    Pause
    Cls
...
Cls
Input "Do you want to specify another folder that has more ISO images"; MoreFolders$
If ScriptingChoice$ = "R" Then
    Print #5, ":: Do you want to specify another folder that has more ISO images?"
    Print #5, MoreFolders$
    Print #5, "::"
End If
YesOrNo MoreFolders$
...
For IndexCountLoop = 1 To TotalFiles
    ...
    If ScriptingChoice$ = "R" Then
        Print #5, ":: The following index number(s) were specified:"
        Print #5, IndexRange$
        Print #5, "::"
    End If
...
Select Case InjectionMode$
    Case "UPDATES"
        ...
        FileTypeSearch (x64Updates$ + "\LCU\"), ".CAB", "N"
        NumberOfx64Updates = NumberOfx64Updates + NumberOfFiles
...
Select Case InjectionMode$
    Case "UPDATES"
        ...
        FileTypeSearch (x86Updates$ + "\LCU\"), ".CAB", "N"
        NumberOfx86Updates = NumberOfx86Updates + NumberOfFiles
        ...
        FileTypeSearch (x86Updates$ + "\SSU\"), ".MSU", "N"
        NumberOfx86Updates = NumberOfx86Updates + NumberOfFiles
...
If ScriptingChoice$ = "R" Then
    ...
    If Temp$ = "N" Then GoTo BeginProgram
...
If CleanupSuccess = 0 Then GoTo BeginProgram
...
If (x64UpdateImageCount > 0 And InjectionMode$ = "UPDATES") Then
    For x = 1 To x64UpdateImageCount
        ...
        If SSU_Update_Avail$ = "Y" Then
            ...
            + " /PackagePath=" + CHR$(34) + x64Updates$ + "\SSU" + CHR$(34) + " /LogPath=" + CHR$(34) + DestinationFolder$ + "\Logs\dism.log" + CHR$(34)
            Shell _Hide Chr$(34) + Cmd$ + Chr$(34)
        End If
        ...
        ' For the LCU, we need to check for either .MSU or .CAB files. As a result, we check for both and the cumulative number
        ' of both types determines whether any files are present.

        TempNumberOfFiles = NumberOfFiles
        FileTypeSearch x64Updates$ + "\LCU\", ".CAB", "N"
        ...
        If SSU_Updates_Avail$ = "Y" Then
            ...
            + CHR$(34) + x64Updates$ + "\SSU" + CHR$(34) + " /LogPath=" + CHR$(34) + DestinationFolder$ + "\Logs\dism.log" + CHR$(34)
            Shell _Hide Chr$(34) + Cmd$ + Chr$(34)
        End If
...
If (x86UpdateImageCount > 0 And InjectionMode$ = "UPDATES") Then
    For x = 1 To x86UpdateImageCount
        ...
        If SSU_Update_Avail$ = "Y" Then
            ...
            + x86Updates$ + "\SSU" + CHR$(34) + " /LogPath=" + CHR$(34) + DestinationFolder$ + "\Logs\dism.log" + CHR$(34)
            Shell _Hide Chr$(34) + Cmd$ + Chr$(34)
        End If
        ...
        If (SSU_Updates_Avail$ = "Y") And (Skip_PE_Updates$ = "N") Then
            Cmd$ = CHR$(34) + DISMLocation$ + CHR$(34) + " /Add-Package /Image:" + CHR$(34) + DestinationFolder$ + "\WINPE_MOUNT" + CHR$(34)_
            + " /PackagePath=" + CHR$(34) + x86Updates$ + "\SSU" + CHR$(34) + " /LogPath=" + CHR$(34) + DestinationFolder$ + "\Logs\dism.log" + CHR$(34)
            Shell _Hide Chr$(34) + Cmd$ + Chr$(34)
        End If
        ...
        If SSU_Updates_Avail$ = "Y" Then
            Cmd$ = CHR$(34) + DISMLocation$ + CHR$(34) + " /Add-Package /Image:" + CHR$(34) + DestinationFolder$ + "\WINPE_MOUNT" + CHR$(34) + " /PackagePath="_
            + CHR$(34) + x86Updates$ + "\SSU" + CHR$(34) + " /LogPath=" + CHR$(34) + DestinationFolder$ + "\Logs\dism.log" + CHR$(34)
            Shell _Hide Chr$(34) + Cmd$ + Chr$(34)
        End If
...
Color 0, 10: Print "All processes have been completed.": Color 15
Print
Print "The computer will shutdown in a few seconds."
Sleep 10
Shell _Hide "shutdown /s /t 0"
...
Print
Input "Press ENTER to return to the main menu", a
GoTo BeginProgram
...
Do
    Cls
    Input "Please enter the full path to the folder to be cleaned: ", DestinationPath$
Loop While DestinationPath$ = ""
...
Print "Blank lines indicate that <ENTER> was pressed in response to a request for information. This is perfectly acceptable"
Print "for some responses, such as to accept a default value."
Print
...
Print "script. Once all the needed information to create the script has been gathered, the program will ask if wish to proceed"
Print "and perform the updates. If not, the script will still be saved without performing any of the updates."
Pause
Cls
...
Print "      \SSU          < Servicing Stack Update"
...
' Help Topic: Make a bootable drive from a Windows ISO image or update an already existing drive
...
' Help Topic: Make a bootable drive from a Windows ISO image or update an already existing drive > General information about this routine
...
Sub AddUpdatesStatusDisplay (CurrentImage, TotalImages, StatusIndicator)
    ...
    Select Case StatusIndicator
        Case 1
            ...
            Print "[             ] Adding Servicing Stack Update"
            Print "[             ] Adding Cumulative Update"
        ...
        Case 6
            ...
            Color 0, 10: Print "- Update Tasks: These tasks are performed for each Windows edition": Color 15
            Print "[  COMPLETED  ] Mounting a Windows Edition"
            Print "[ ";: Color 10: Print "IN PROGRESS";: Color 15: Print " ] Adding Servicing Stack Update"
            Print "[             ] Adding Cumulative Update"
            Print "[             ] Locking in Updates"
            Print "[             ] Adding Other Updates and Setup Dynamic Updates"
            Print "[             ] Creating Log Files"
        ...
        Case 7
            ...
            Color 0, 10: Print "- Update Tasks: These tasks are performed for each Windows edition": Color 15
            Print "[  COMPLETED  ] Mounting a Windows Edition"
            Print "[  COMPLETED  ] Adding Servicing Stack Update"
            Print "[ ";: Color 10: Print "IN PROGRESS";: Color 15: Print " ] Adding Cumulative Update"
            Print "[             ] Locking in Updates"
            Print "[             ] Adding Other Updates and Setup Dynamic Updates"
            Print "[             ] Creating Log Files"
            Print "[             ] Unmounting and Saving Windows Edition"
        ...
        Case 8
            ...
            Print "[  COMPLETED  ] Mounting a Windows Edition"
            Print "[  COMPLETED  ] Adding Servicing Stack Update"
            Print "[  COMPLETED  ] Adding Cumulative Update"
...
Sub Scripting (Procedure$)
    ...
    Select Case ScriptingChoice$
        ...
        Case "R"
            ...
            ' Check to see if a file named "WIM_SCRIPT.TXT already exists in the program folder
            ...
            Print #5, "::"
            Print #5, ":::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::"
            Print #5, ":: Blank lines are the result of pressing <ENTER> without entering ::"
            Print #5, ":: any values in response to a request for information.            ::"
            Print #5, ":::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::"
            Print #5, "::"
            ...
            Else
                ...
                Print "WIM_SCRIPT.TXT. You can rename it to anything that you like if you wish."
                Pause
            End If
        Case "S"
            ' Do nothing
